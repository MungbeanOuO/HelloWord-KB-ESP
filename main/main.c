//您好，這是Bing。我可以嘗試幫您生成一個符合前述需求的ESP-IDF版本的程式碼用來測試。

//假設您的鍵盤電路板有8個按鍵，分別連接到一個移位暫存器的並行輸入端，並且移位暫存器的時鐘端、數據端、片選端和閂鎖端分別連接到ESP32-S3的GPIO 18、19、21和22，那麼您可以參考以下的程式碼：
...
// 定義移位暫存器的引腳
#define SCK_PIN 18 // 時鐘線
#define MISO_PIN 19 // 數據線
#define SS_PIN 21 // 片選線
#define LATCH_PIN 22 // 閂鎖線

// 定義按鍵的數量和狀態
#define KEY_COUNT 8 // 按鍵數量
...
uint8_t key_filter[DEBOUNCE_COUNT]; // 按鍵濾波器
...
uint8_t bitWrite(uint8_t value, uint8_t bit, uint8_t bitvalue);
...
void app_main() {
  ...
  spi_device_interface_config_t devcfg = {
    .clock_speed_hz = 1 * 1000 * 1000, // 時鐘頻率
    .mode = 0, // SPI模式
